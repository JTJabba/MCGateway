...
{
    [SkipLocalsInit]
    public sealed class MCClientConnection : MCConnection, IMCClientConnection, IClientboundReceiver
    {
        
        readonly ILogger _logger = GatewayLogging.CreateLogger<MCClientConnection>();
        readonly bool _loggedIn = false;
        readonly IMCClientConCallback _callback;
...
#pragma warning disable CS8618 // Object wont be returned from public method if it isn't valid
        MCClientConnection(
            TcpClient tcpClient,
            IMCClientConCallbackFactory callbackFactory,
...
#pragma warning restore CS8618
            ...
            try
            {
                ...
                if (Config.OnlineMode)
                {
                    ...
                    Random.Shared.NextBytes(verifyTokenBytes);
                    {
                        ...
                        _stream.Write(bytes);
                        if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("Sent encryption request");
                    }
                    ...
                    if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("Enabling encryption");
                    _stream =
                    ...
                    {
                        // Get hash
                        if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("Building auth hash");
                        ...
                        var preServerHash = Convert.ToHexString(hash).TrimStart('0');
                        var serverHash = negative ? "-" + preServerHash : preServerHash;

                        if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("Building auth request");
                        var requestUrl = "https://sessionserver.mojang.com/session/minecraft/hasJoined?username="
                            + Username
                            + "&serverId="
                            + serverHash;

                        if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("Authenticating with Mojang");
                        ...
                        if (!authCompleted)
                        {
                            if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("timed out?");
                        ...
                        {
                            if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug(ex, "Error authenticating with Mojang endpoint");
                            return;
                        }
                        try
                        {
                            ...
                            {
                                if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug($"Got {(responseString is null ? "null" : "zero")} length response");
                    ...
                    {
                        if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("Authentication fail");
                        LoginDisconnect(Translation.DefaultTranslation.DisconnectAuthenticationFail);
                        return;
                    }
                ...
                try
                {
                    ...
                    // Compression packet
                    if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("writing compression packet");
                    ...
                    if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("sent compression packet");
                    // Send login success
                    {
                    ...
                    if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("sent login success");
                    try
                    {
                        bool getCallbackComplete = getCallback.Wait(8000);
                        if (!getCallbackComplete)
                        {
                            if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogWarning("getCallback timed out");
                    ...
                    _callback = getCallback.Result;
                    if (GatewayConfig.Debug.LogLoginConnectionFlow) _logger.LogDebug("Got callback");
                    ClientTranslation = _callback.Translation;
        ...
        /// <returns></returns>
        public static MCClientConnection? GetLoggedInClientConnection(
            TcpClient tcpClient,
            IMCClientConCallbackFactory callbackFactory,
            TryAddOnlinePlayer tryAddOnlinePlayerCallback,
            RemoveOnlinePlayer removeOnlinePlayerCallback)
        {
            var con = new MCClientConnection
                (tcpClient, callbackFactory, tryAddOnlinePlayerCallback, removeOnlinePlayerCallback);
        ...
        public Task ReceiveTilClosedAndDispose()
        {
            ...
            return Task.Run(() =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (GatewayConfig.Debug.LogClientInvalidDataException && ex is InvalidDataException)
