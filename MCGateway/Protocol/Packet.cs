...
public ref struct Packet
{
    ...
    public static int ReadVarIntWithLength(ReadOnlySpan<byte> buffer, out int bytesRead)
    {
        const int SEGMENT_MASK = 0x7F;
        const int CONTINUE_MASK = 0x80;
        const int MAX_VARINT_LENGTH = 32;
    ...
    {
        int strLength = ReadVarIntWithLength(buffer, out int strLengthBytes);
        bytesRead = strLengthBytes + strLength;
        return Encoding.UTF8.GetString(buffer.Slice(strLengthBytes, strLength));
    }
